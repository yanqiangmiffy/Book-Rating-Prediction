...
def w2v_feat(data_frame, feat, mode):
    ...
    for m, t in enumerate(feat):
        ...
        tmp_df = data_frame[['event_id', t]].merge(tmp_df, on=t)
        tmp_df = tmp_df.drop_duplicates().groupby('event_id').agg(stat_list).reset_index()
        tmp_df.columns = ['event_id'] + [f'{p}_{q}' for p in w2c_list for q in stat_list]
        if m == 0:
            new_all = pd.concat([new_all, tmp_df], axis=1)
        else:
            new_all = pd.merge(new_all, tmp_df, how='left', on='event_id')
    return new_all
...
if update:
    ...
    # data=w2v_feat(data,['User-ID','ISBN','Location','Age','Book-Author','Year-Of-Publication','Publisher'],'data')
    del data['Book-Title']
...
no_fea = ['Book-Title', 'id', 'Book-Rating', 'ID',
          'fold', 'Book-Author', 'Publisher', 'Location', 'User-ID']
