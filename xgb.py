...
for num_classes in nums:
    ...
    test[['id', 'Book-Rating']].to_csv('result/xgb_numclasses{}_{}.csv'.format(num_classes, score), index=False,
                                       header=False)
    print(test['Book-Rating'].value_counts())

    pd.DataFrame(result_dict_lgb['oof']).to_csv('result/xgb_numclasses{}_{}trainoof.csv'.format(num_classes, score),
                                                index=False, header=False)
    pd.DataFrame(result_dict_lgb['prediction']).to_csv(
        'result/xgb_numclasses{}_{}testoof.csv'.format(num_classes, score),
        index=False, header=False)
